...
def _make_overlay():
    ...
    overlay = layer._overlays['lasso']
    vispy = VispyLabelsLassoOverlay(
        layer=layer,
        font_info=FontInfo(),
        viewer=viewer,
        overlay=overlay,
    )
    layer.mode = 'lasso'  # enables the overlay
...
def _press(MouseEvent, layer, position):
    event = MouseEvent(
        type='mouse_press', button=1, position=position, dims_displayed=(0, 1)
    )
    event.modifiers = ()
...
def test_lasso_circle_appears_on_click(MouseEvent):
...
def test_click_during_stroke_pans_then_resumes(MouseEvent):
    ...
    _press(MouseEvent, layer, (15, 15))
    assert overlay.active is True
    assert layer.mouse_pan is False  # lasso mode default

    # a (non-left) press during the stroke enables camera panning
