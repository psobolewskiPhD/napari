...
if TYPE_CHECKING:
    from napari.components.overlays import LabelsLassoOverlay
...
def _only_when_overlay_enabled(callback):
    ...
    active during LASSO mode. When editing is not 2D we silently no-op rather
    than switching mode.
    """
...
class VispyLabelsLassoOverlay(LayerOverlayMixin, VispySceneOverlay):
    layer: Labels
    overlay: LabelsLassoOverlay
    ...
    def _on_mouse_press(self, layer, event):
        if not self.overlay.active:
            # the left button starts an encircle-and-fill stroke
            if event.button == 1:
    ...
    def _on_mouse_move(self, layer, event):
        if not self.overlay.active:
            return
        # don't paint while the user is adjusting brush size (Alt+move)
